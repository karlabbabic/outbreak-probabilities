...
MODEL_DIR = Path(__file__).resolve().parent / "models_4weeks"
...
def predict_pmo(model_name: str, week_1: float, week_2: float, week_3: float, week_4: float, threshold: float = 0.5, model_dir: Path = MODEL_DIR):
    """
    ...
    X_new = np.array([[float(week_1), float(week_2), float(week_3), float(week_4)]])
...
def main(argv=None):
    parser = argparse.ArgumentParser(description="Train RF/GB PMO classifiers on 4-week-ahead simulated data.")
    parser.add_argument("--data", type=str, default=DEFAULT_DATA_PATH, help=f"Input simulations CSV (default: {DEFAULT_DATA_PATH})")
    parser.add_argument("--model-dir", type=str, default=str(MODEL_DIR), help="Directory to write trained models (default: package's models_4weeks directory)")
