...
def predict_pmo(pipeline: Pipeline, model_name: str, week_1: float, week_2: float, week_3: float, week_4: float, threshold: float = 0.5) -> dict:
    """
    ...
    X_new = pd.DataFrame([[float(week_1), float(week_2), float(week_3), float(week_4)]], columns=["week_1", "week_2", "week_3", "week_4"])
...
def run_training_pipeline(data_path: Path, model_dir: Path):
    ...
    X = cleaned_data[["week_1", "week_2", "week_3", "week_4"]].astype(float)
...
if __name__ == "__main__":
    ...
    try:
        ...
        prediction_result = predict_pmo(loaded_rf_pipeline, "RF", week_1=1, week_2=0, week_3=1, week_4=0)
