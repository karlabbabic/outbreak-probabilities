...
def clean_simulation_data(raw_df: pd.DataFrame) -> pd.DataFrame:
    ...
    """
    df = raw_df.copy()
    expected_columns = ["week_1", "week_2", "PMO"]
...
def predict_pmo(pipeline: Pipeline, model_name: str, week_1: float, week_2: float, threshold: float = 0.5) -> dict:
    """
    ...
    X_new = pd.DataFrame([[float(week_1), float(week_2)]], columns=["week_1", "week_2"])
...
def run_training_pipeline(data_path: Path, model_dir: Path):
    ...
    X = cleaned_data[["week_1", "week_2"]].astype(float)
...
if __name__ == "__main__":
    ...
    try:
        loaded_rf_pipeline = load_pipeline(MODEL_DIR, "RF", n_weeks=2)
        prediction_result = predict_pmo(loaded_rf_pipeline, "RF", week_1=0.85, week_2=1.42)
